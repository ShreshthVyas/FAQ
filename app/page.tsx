...
export default function Home() {
  ...
  const handleSearch = async () => {
    try {
      ...
      setShowQuickLinks(false); // Hide QuickLinks when search is performed
    } catch (error) {
      if (
        typeof error === "object" &&
        error &&
  ...
  const handleInputChange = (event:any) => {
    ...
    if (event.target.value === '') {
      setShowQuickLinks(true); // Show QuickLinks when search term is reset to empty
    }
  };
  ...
  return (
    <main className="flex flex-col items-center justify-center h-screen bg-black">
      <div className="h-[20rem] w-full bg-black flex flex-col items-center justify-center overflow-hidden rounded-md mt-16">
      ...
      <div className="flex flex-col items-center rounded-2xl ">
        ...
        />
        <Button onClick={handleSearch} className="text-white rounded-xl mb-4 px-4 py-2 text-sm">
          Search
        </Button>
      ...
      {showPopup && (
        <div className="mt-24 fixed flex items-center justify-center z-50 rounded-xl px-8">
          <div className="flex flex-col items-center bg-blue-500 bg-opacity-500 p-8 rounded-xl shadow-md text-white text-center px-4">
            ...
            <button onClick={handlePopupClose} className="rounded-2xl mt-4 px-4 py-2 bg-white text-blue-500">
              Okay
            </button>
          </div>
        </div>
